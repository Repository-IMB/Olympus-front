...
import estilos from "./Vacaciones.module.css";
import type { ColumnsType } from "antd/es/table";
import type { Vacacion } from "../../interfaces/IVacacion";

const { Option } = Select;
const { RangePicker } = DatePicker;

// Mock data - reemplazar con API real
const vacacionesMock: Vacacion[] = [
  {
    id: 1,
    pais: "Panamá",
    dni: "88391694",
    ...
    celular: "",
    vacaciones: 0,
    primera: "imbpaegermtto08252",
    segunda: "imbpaegermtto08252",
    tercera: "",
    cuarta: "",
  ...
  },
  {
    ...
    celular: "",
    vacaciones: 0,
    primera: "imbcepconfin06251",
    segunda: "imbcepconfin06251",
    tercera: "",
    cuarta: "",
...
export default function Vacaciones() {
  ...
  const [vacaciones] = useState<Vacacion[]>(vacacionesMock);
  ...
  const vacacionesFiltradas = useMemo(() => {
    ...
    return vacaciones.filter((v) => {
      const coincideBusqueda =
        ...
        v.apellidos.toLowerCase().includes(busqueda) ||
        v.dni.includes(busqueda) ||
        v.correo.toLowerCase().includes(busqueda);

      const coincidePais = !filtroPais || v.pais === filtroPais;
      const coincideSede = !filtroSede || v.sede === filtroSede;
      const coincideCarrera = !filtroCarrera || v.carrera === filtroCarrera;
      const coincideTipoContrato = !filtroTipoContrato || v.tipoContrato === filtroTipoContrato;
      const coincideTipoJornada = !filtroTipoJornada || v.tipoJornada === filtroTipoJornada;
      ...
      const coincideFecha =
        ...
        !dateRange[1] ||
        (() => {
          if (!v.fechaNacimiento) return true;
          const [inicio, fin] = dateRange;
          const fechaVacacion = moment(v.fechaNacimiento, "DD/MM/YYYY");
          return (
            (fechaVacacion.isAfter(inicio.startOf("day")) &&
              fechaVacacion.isBefore(fin.endOf("day"))) ||
            fechaVacacion.isSame(inicio, "day") ||
            fechaVacacion.isSame(fin, "day")
          );
        })();
  ...
  }, [vacaciones, searchText, filtroPais, filtroSede, filtroCarrera, filtroTipoContrato, filtroTipoJornada, filtroArea, dateRange]);

  const columnas: ColumnsType<Vacacion> = [
    {
      title: "País",
      dataIndex: "pais",
      key: "pais",
    ...
    },
    {
      ...
      key: "vacaciones",
      sorter: (a, b) => a.vacaciones - b.vacaciones,
      filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div style={{ padding: 8 }}>
          <Input
            placeholder="Filtrar vacaciones"
            value={selectedKeys[0] as string}
            onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={() => confirm()}
            style={{ width: 188, marginBottom: 8, display: "block" }}
      ...
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#1890ff" : undefined }} />,
      onFilter: (value, record) => record.vacaciones.toString().includes(value as string),
    },
    {
      title: "Primera",
      dataIndex: "primera",
      ...
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#1890ff" : undefined }} />,
      onFilter: (value, record) => record.primera.toLowerCase().includes((value as string).toLowerCase()),
    },
    {
      title: "Segunda",
      dataIndex: "segunda",
      key: "segunda",
      sorter: (a, b) => a.segunda.localeCompare(b.segunda),
      filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div style={{ padding: 8 }}>
          <Input
            placeholder="Filtrar segunda"
            value={selectedKeys[0] as string}
            onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={() => confirm()}
            style={{ width: 188, marginBottom: 8, display: "block" }}
      ...
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#1890ff" : undefined }} />,
      onFilter: (value, record) => record.segunda.toLowerCase().includes((value as string).toLowerCase()),
    },
    {
      title: "Tercera",
      dataIndex: "tercera",
      key: "tercera",
      sorter: (a, b) => a.tercera.localeCompare(b.tercera),
      filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div style={{ padding: 8 }}>
          <Input
            placeholder="Filtrar tercera"
            value={selectedKeys[0] as string}
            onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={() => confirm()}
            style={{ width: 188, marginBottom: 8, display: "block" }}
      ...
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#1890ff" : undefined }} />,
      onFilter: (value, record) => record.tercera.toLowerCase().includes((value as string).toLowerCase()),
    },
    {
      title: "Cuarta",
      dataIndex: "cuarta",
      key: "cuarta",
      sorter: (a, b) => a.cuarta.localeCompare(b.cuarta),
      filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, clearFilters }) => (
        <div style={{ padding: 8 }}>
          <Input
            placeholder="Filtrar cuarta"
            value={selectedKeys[0] as string}
            onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
            onPressEnter={() => confirm()}
            style={{ width: 188, marginBottom: 8, display: "block" }}
      ...
      filterIcon: (filtered) => <SearchOutlined style={{ color: filtered ? "#1890ff" : undefined }} />,
      onFilter: (value, record) => record.cuarta.toLowerCase().includes((value as string).toLowerCase()),
    },
    {
      title: "Accion",
      key: "accion",
  ...
  const paisesUnicos = [...new Set(vacaciones.map((v) => v.pais))].filter(Boolean);
  const sedesUnicas = [...new Set(vacaciones.map((v) => v.sede))].filter(Boolean);
  const carrerasUnicas = [...new Set(vacaciones.map((v) => v.carrera))].filter(Boolean);
  const tiposContratoUnicos = [...new Set(vacaciones.map((v) => v.tipoContrato))].filter(Boolean);
  const tiposJornadaUnicos = [...new Set(vacaciones.map((v) => v.tipoJornada))].filter(Boolean);
  const areasUnicas = [...new Set(vacaciones.map((v) => v.area))].filter(Boolean);

  return (
    <div className={estilos.container}>
      <div className={estilos.contentWrapper}>
        <div className={estilos.header}>
          <h1 className={estilos.title}>Vacaciones</h1>
        </div>
        ...
        <Table
          columns={columnas}
          dataSource={vacacionesFiltradas}
          rowKey="id"
